...
Under `sql/`:
- place here any (Postgre)SQL-based transformation scripts. The provided `sample_script.sql` contains the code to extract 
`observation_period` information from existing CDM tables, including the custom `TreatmentLine` table. Edit or remove as needed.
